...
JNIEXPORT void Java_org_telegram_messenger_Utilities_generateBackgroundBitmap(JNIEnv *env, jclass clazz, jobject bitmap, jfloatArray jpoints, jintArray jcolors) {
    AndroidBitmapInfo info;
    if (AndroidBitmap_getInfo(env, bitmap, &info) != ANDROID_BITMAP_RESULT_SUCCESS) {
        return;
    }
    ...
    jfloat* points = env->GetFloatArrayElements(jpoints, 0);
    jint* colors = env->GetIntArrayElements(jcolors, 0);

    int x, y;
    auto *ds = new float[4];
    auto *rs = new uint8_t[4];
    auto *gs = new uint8_t[4];
    auto *bs = new uint8_t[4];
    ...
    for (x = 0; x < w; x++) {
        ...
        for (y = 0; y < h; y++) {
            float ry = (float) y / h;
            for (int i = 0; i < 4; i++) {
                ds[i] = sqrt(pow(points[i * 2] - rx, 2) + pow(points[i * 2 + 1] - ry, 2));
                rs[i] = (uint8_t) ((colors[i] >> 16) & 0xff);
                gs[i] = (uint8_t) ((colors[i] >> 8) & 0xff);
                bs[i] = (uint8_t) (colors[i] & 0xff);
            }
            ...
            int offset = stride * y + x * 4;
            pixels[offset] = rs[0] * ds[0] + rs[1] * ds[1] + rs[2] * ds[2] + rs[3] * ds[3];
            pixels[offset + 1] = gs[0] * ds[0] + gs[1] * ds[1] + gs[2] * ds[2] + gs[3] * ds[3];
            pixels[offset + 2] = bs[0] * ds[0] + bs[1] * ds[1] + bs[2] * ds[2] + bs[3] * ds[3];
            pixels[offset + 3] = 255;
        }
    }

    env->ReleaseFloatArrayElements(jpoints, points, 0);
